...
countryInput.addEventListener("input", (0, _lodashDebounceDefault.default)(inputCountry, DEBOUNCE_DELAY));
function inputCountry(evt) {
    const nameCountry = evt.target.value.trim();
    if (nameCountry === "") return;
    (0, _fetchCountries.fetchCountries)(nameCountry).then((response)=>{
        if (response.length > 10) (0, _notiflixNotifyAio.Notify).info("Too many matches found. Please enter a more specific name.");
        if (response.length >= 2 && response.length <= 10) searchListCountry(response);
        if (response.length === 1) searchOneCountry(response);
...
function searchOneCountry(response) {
    const markup = response.map((el)=>{
        return `<div class="item_country"><img class="img" src="${el.flags.svg}" width=50 alt="flag">
...
},{"./css/styles.css":"1CY4s","notiflix/build/notiflix-notify-aio":"eXQLZ","lodash.debounce":"3JP5n","./fetchCountries":"fTQqv","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1CY4s":[function() {},{}],"eXQLZ":[function(require,module,exports) {
var global = arguments[3];
/*
...
function fetchCountries(name) {
    ...
    return fetch(`https://restcountries.com/v3.1/name/${name}?fields=${searchCountry}`).then((response)=>{
        if (!response.ok) throw new Error((0, _notiflixNotifyAio.Notify).failure("Oops, there is no country with that name"));
        return response.json();
    });
}

},{"notiflix/build/notiflix-notify-aio":"eXQLZ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gkKU3":[function(require,module,exports) {
